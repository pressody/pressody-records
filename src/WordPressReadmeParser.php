 ...
 * WordPress.org readme(.txt) parser.
 *
 * Parse a plugin or theme readme and extract the header information, without the actual markdown since we don't need it right now.
 *
 * This is a modified version of this: https://github.com/SkyPressATX/WordPress-Plugin-Readme-Parser/blob/master/parse-readme.php
 *
 * @since 0.5.0
 */
class WordPressReadmeParser {
	 ...
	 * @return array
	 */
	public function parse_readme( string $file ): array {
	 ...
	 * @return array
	 */
	public function parse_readme_contents( string $file_contents ): array {
		...
		// Requires PHP: 5.2.4
		if ( preg_match( '|Requires PHP:(.*)|i', $file_contents, $requires_php ) ) {
			$requires_php = $this->sanitize_text( $requires_php[1] );
		} else {
			$requires_php = null;
		}
		...
		// togs, contributors, etc are optional and order shouldn't matter.  So we chop them only after we've grabbed their values.
		foreach (
			array(
				'tags',
				'contributors',
		...
		for ( $i = 1; $i <= count( $_sections ); $i += 2 ) {
			...
			$_sections[ $i ]                                           = $this->filter_text( $_sections[ $i ] );
		...
		if ( ! isset( $final_sections['description'] ) ) {
			$final_sections = array_merge( array( 'description' => $this->filter_text( $_short_description[2] ) ), $final_sections );
			$excerpt        = true;
		}
	 ...
	 * @return string
	 */
	protected function chop_string( string $string, string $chop ): string {
	 ...
	 * @return string
	 */
	protected function user_sanitize( string $text, bool $strict = false ): string {
		// whitelisted chars
	...
	protected function filter_text( string $text ): string {
		...
		$text = call_user_func( array(
			__CLASS__,
			'code_trick',
		), $text ); // A better parser than Markdown's for: backticks -> CODE
	 ...
	 * @return string
	 */
	protected function encodeit( array $matches ): string {
	 ...
	 * @return string
	 */
	protected function decodeit( array $matches ): string {
